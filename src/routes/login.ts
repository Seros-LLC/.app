...
export async function loginPage(req: Request, res: Response) {
  ...
  // visitor to keep retrying a button that cannot work is the worse outcome.
  const errBanner =
      err === 'captcha_failed'
    ...
    ? notice('warn', 'Google sign-in is off in this environment',
        'Use your email address and password below, or sign in with GitHub.')
    : err === 'github_not_configured'
    ? notice('warn', 'GitHub sign-in is off in this environment',
        'Use your email address and password below.')
    : err
