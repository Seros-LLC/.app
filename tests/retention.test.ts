...
function seedMessage(s: WorkspaceScope, channelId: string, ts: string, body: string) {
  const { row } = s.ingestMessage({ channelId, ts, authorId: 'u-ana', body });
  return row;
}

test('retention: content is purged after the window and identifiers survive', () => {
  const s = WorkspaceScope.ensure(db, 'R-purge');
  const row = seedMessage(s, 'C1', '100.1', 'the secret quarterly numbers are 42');
  const hashBefore = row.bodyHash;

  const r = sweepWorkspace(db, 'R-purge', { now: daysLater(31) });
...
test('retention: content is NOT purged before the window', () => {
  const s = WorkspaceScope.ensure(db, 'R-young');
  const row = seedMessage(s, 'C1', '200.2', 'still inside the retention window');

  const r = sweepWorkspace(db, 'R-young', { now: daysLater(29) });
...
test('retention: honours the per-workspace retentionContentDays policy', () => {
  const s = WorkspaceScope.ensure(db, 'R-policy');
  db.update(workspaces).set({ retentionContentDays: 7 }).where(eq(workspaces.id, 'R-policy')).run();
  const row = seedMessage(s, 'C1', '300.3', 'short window content');

  const early = sweepWorkspace(db, 'R-policy', { now: daysLater(6) });
  ...
  const late = sweepWorkspace(db, 'R-policy', { now: daysLater(8) });
  assert.equal(late.counts.source_messages, 1);
  assert.equal(msg('R-policy', row.id)!.body, null);
});

test('retention: the sweep writes a retention.swept audit row carrying counts only', () => {
  const s = WorkspaceScope.ensure(db, 'R-audit');
  seedMessage(s, 'C1', '400.4', 'salary review for Ana is confidential');

  const before = auditFor('R-audit', 'retention.swept').length;
  const r = sweepWorkspace(db, 'R-audit', { now: daysLater(31) });
...
test('retention: audit rows and meter rows survive the sweep', () => {
  const s = WorkspaceScope.ensure(db, 'R-survive');
  s.meter('detect', 'ok');
  seedMessage(s, 'C1', '500.5', 'content that will be purged');
  ...
  sweepWorkspace(db, 'R-survive', { now: daysLater(31) });
...
test('retention: sweeping twice changes nothing the second time (idempotent)', () => {
  const s = WorkspaceScope.ensure(db, 'R-idem');
  const row = seedMessage(s, 'C1', '600.6', 'purge me exactly once');

  const first = sweepWorkspace(db, 'R-idem', { now: daysLater(31) });
  const afterFirst = msg('R-idem', row.id)!;
  assert.equal(first.counts.source_messages, 1);

  const second = sweepWorkspace(db, 'R-idem', { now: daysLater(32) });
...
test('retention: sweepAllWorkspaces refuses an unknown workspace and covers known ones', () => {
  assert.throws(() => sweepWorkspace(db, 'R-does-not-exist'), UnknownWorkspace);
  const results = sweepAllWorkspaces(db, { now: NOW });
  assert.ok(results.length > 0);
  assert.ok(results.some((r) => r.workspaceId === 'R-purge'));
  for (const r of results) assert.equal(r.rowsPastWindowAfterSweep, 0);
});

test('disconnect: tokens destroyed, pending drafts expired and never confirmed', () => {
  const s = WorkspaceScope.ensure(db, 'R-disc');
  s.addMember('u-me', 'Me');
  const row = seedMessage(s, 'C-slack', '700.7', "I'll ship the migration by Friday.");
...
test("disconnect: that source's content is purged inside the 24h SLA, and is idempotent", () => {
  const s = WorkspaceScope.ensure(db, 'R-disc2');
  const mine = seedMessage(s, 'C-gone', '800.8', 'content belonging to the disconnected source');
  const other = seedMessage(s, 'C-stays', '801.8', 'content from a different connection');
  const connId = registerConnection(db, 'R-disc2', {
    kind: 'slack', provider: 'slack', externalAccountId: 'T-ext2',
    channelIds: ['C-gone'], accessToken: 'xoxb-secret',
  });

  const r = disconnectConnection(db, 'R-disc2', connId, { now: NOW });
  assert.equal(r.ok, true);
  if (!r.ok) return;
  ...
  const again = disconnectConnection(db, 'R-disc2', connId, { now: NOW + 1000 });
...
test('disconnect: an unknown connection is refused explicitly, never silently confirmed', () => {
  WorkspaceScope.ensure(db, 'R-disc3');
  const r = disconnectConnection(db, 'R-disc3', 'no-such-connection');
...
test('workspace deletion: tenant rows go, audit trail and meter stay, status is deleted', () => {
  const s = WorkspaceScope.ensure(db, 'R-del');
  s.addMember('u-me', 'Me');
  s.meter('draft', 'ok');
  const row = seedMessage(s, 'C1', '900.9', 'delete this workspace content');
  const draftId = s.createDraft({
  ...
  const confirmed = s.confirm(draftId, 'confirmed', 'u-me');
  assert.equal(confirmed.ok, true);
  registerConnection(db, 'R-del', {
  ...
  const r = deleteWorkspace(db, 'R-del', { now: NOW });
...
test('workspace deletion: deleting twice is idempotent and other tenants are untouched', () => {
  const neighbour = WorkspaceScope.ensure(db, 'R-neighbour');
  const kept = seedMessage(neighbour, 'C1', '950.9', 'a different tenant, still here');
  const s = WorkspaceScope.ensure(db, 'R-del2');
  seedMessage(s, 'C1', '951.9', 'doomed content');

  deleteWorkspace(db, 'R-del2', { now: NOW });
  const again = deleteWorkspace(db, 'R-del2', { now: NOW + 1 });
  ...
  const swept = sweepWorkspace(db, 'R-del2', { now: daysLater(60) });
